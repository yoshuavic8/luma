...
export default function SermonBuilderPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Error handling
      setError(error instanceof Error ? error.message : 'Failed to generate sermon outline. Please try again.');
    } finally {
    ...
    try {
      ...
      if (pdf.getStringUnitWidth(scripture) * 12 / pdf.internal.scaleFactor > contentWidth) {
        ...
        for (let i = 0; i < words.length; i++) {
          ...
          if (testWidth > contentWidth * 0.8 && i > 0) { // Use 80% of content width for centered text
      ...
      if (generatedOutline.hook) {
        ...
        pdf.rect(margin, yPos, contentWidth, 10 + (generatedOutline.hook.length / 50) * lineHeight, 'F');
      ...
      pdf.rect(margin, yPos, contentWidth, 10 + (generatedOutline.introduction.length / 50) * lineHeight, 'F');
      ...
      generatedOutline.mainPoints.forEach((point, index) => {
        ...
        const pointHeight = 15 + ((point.explanation.length + (point.scripture?.length || 0)) / 50) * lineHeight;
      ...
      pdf.rect(margin, yPos, contentWidth, 10 + (generatedOutline.conclusion.length / 50) * lineHeight, 'F');
      ...
      if (generatedOutline.biblicalSolution) {
        ...
        const solutionHeight = 15 + ((solutionText.length + illustrationText.length) / 50) * lineHeight;
      ...
      if (generatedOutline.applicationPoints && generatedOutline.applicationPoints.length > 0) {
        ...
        const pointsHeight = 15 + (generatedOutline.applicationPoints.join(' ').length / 50) * lineHeight;
      ...
      if (generatedOutline.personalChallenge) {
        ...
        const challengeHeight = 15 + ((challengeText.length + illustrationText.length) / 50) * lineHeight;
      ...
      const filename = generatedOutline.title
  ...
  const memoizedLoadSavedOutlines = useCallback(() => {
    ...
    if (userData) {
      ...
      try {
        storage.getSermonOutlines(userData)
          .then(outlines => {
                  ...
                  <SheetHeader>
                    <div className="flex justify-between items-center">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => debouncedLoadOutlinesRef.current && debouncedLoadOutlinesRef.current()}
                        disabled={isLoading}
                          ...
                          <CardContent className="p-4">
                            <div className="flex justify-between items-start">
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                className={`absolute top-2 right-2 ${deleteStatus?.id === outline.id ?
                                  (deleteStatus?.status === 'deleting' ? 'text-amber-500' :
                                   deleteStatus?.status === 'success' ? 'text-green-500' :
                                   deleteStatus?.status === 'error' ? 'text-red-500' : 'text-gray-400') :
          ...
          <div className="grid gap-4 md:gap-6 grid-cols-1 md:grid-cols-2">
              ...
              <CardContent className="space-y-6">
                ...
                {userData?.tier === 'free' && (
                  <div className="rounded-md bg-blue-50 p-3 text-sm">
                    <p className="font-medium text-blue-700">Free Tier: {userData.usageCount || 0}/5 generations today</p>
                    <p className="mt-1 text-blue-600">
                      <Link href="/subscription" className="underline">Upgrade to Pro</Link> for unlimited generations and more features.
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <select
                    id="audience"
                    value={audience}
                    onChange={(e) => setAudience(e.target.value as 'general' | 'youth' | 'children' | 'seniors')}
            ...
            <Card className="h-full">
              <CardHeader>
                ...
                <CardDescription>
                  Your AI-generated sermon outline will appear here
                </CardDescription>
              </CardHeader>
                ...
                {isGenerating ? (
                  ...
                  <div className="space-y-6 outline-container">
                    ...
                    <div className="space-y-2 border-b pb-4">
                      <h2 className="text-xl font-bold text-blue-800 outline-title">{generatedOutline.title}</h2>
                      <p className="text-sm text-gray-600 italic outline-scripture">{generatedOutline.scripture}</p>
                    </div>
                    ...
                    <div className="space-y-4 outline-section">
                      <h3 className="font-medium text-gray-700 border-b pb-1 section-header">Main Points</h3>
                      {generatedOutline.mainPoints.map((point, index) => (
                        <div key={index} className="space-y-2 p-3 bg-green-50 rounded-md border border-green-100 main-point">
                          <h3 className="font-medium text-green-800 flex items-center main-point-title">
                            <span className="flex w-5 h-5 bg-green-600 text-white rounded-full mr-2 items-center justify-center text-xs">{index + 1}</span>
                            {point.title}
                          </h3>
                          <p className="text-xs text-gray-600 italic ml-7 main-point-scripture">{point.scripture}</p>
                        ...
                        <ul className="list-inside space-y-2 ml-2">
                          {generatedOutline.applicationPoints.map((point, index) => (
                            <li key={index} className="text-sm flex items-start application-point">
                              <span className="flex w-5 h-5 bg-amber-600 text-white rounded-full mr-2 flex-shrink-0 items-center justify-center text-xs">{index + 1}</span>
                              <span>{point}</span>
                            </li>
                          ))}
                        </ul>
                    ...
                    {generatedOutline.personalChallenge && (
                      <div className="space-y-2 p-3 bg-rose-50 rounded-md border border-rose-100 outline-section">
                        ...
                        <div className="space-y-2">
                          <p className="text-sm font-medium">{generatedOutline.personalChallenge.challenge}</p>
                  ...
                  <div className="flex h-64 items-center justify-center">
                    <p className="text-center text-gray-500">
                      Fill in the form and click &quot;Generate Outline&quot; to create a sermon outline
                    </p>
                  </div>
                )}
              </CardContent>
